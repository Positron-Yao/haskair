...
opts =
  info
    (configParser <**> helper)
    (fullDesc <> progDesc "Get weather information from weatherapi.com" <> header "haskair - Haskell weather app")
    ...
    Right tomltext -> do
      ...
      case eitherToml of
        ...
        Right toml -> do
          ...
          response <-
            let a = (maybeS (jtoml ^? key "api_key" . _String))
                c =
                  if null (city config)
                    ...
                    Just current -> do
                      ...
                      putStrLn $
                        (fmt' (maybeS (trans ^? key "current" . key "wind" . _String)))
                          ++ maybeN (current ^? key "wind_kph" . _Number)
                          ++ " km/h"
                      putStrLn $
                    ...
                    Just forecast -> do
                      ...
                      putStrLn $
                        (fmt' (maybeS (trans ^? key "forecast" . key "precip" . _String)))
                          ++ maybeN (forecast ^? key "totalprecip_mm" . _Number)
                          ++ "mm"
                      putStrLn $
                        (fmt' (maybeS (trans ^? key "forecast" . key "snow" . _String)))
                          ++ maybeN (forecast ^? key "totalsnow_cm" . _Number)
                          ++ "mm"
